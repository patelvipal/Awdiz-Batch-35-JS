...
const gstIncludedPrice = products.map((price) => price * 2.88);
console.log(gstIncludedPrice);

const cart = [200, 100, 50];
const taxPrice = cart.map((price) => price * 2.78);
...
const usersData = [
  { name: "Vipal", active: true },
  { name: "Jay", active: false },
  { name: "Ajay", active: true },
];

const activePlayers = usersData.filter((player) => player.active);
